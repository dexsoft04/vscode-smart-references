...
`References` 视图标题栏支持 `$(filter)` 过滤按钮，可在 `All / Production / Tests / Current File / Current Directory / Workspace Source` 之间切换。
...
### Symbol Search（Shift+Shift / Ctrl+Shift+N / Ctrl+Alt+T）
...
独立于符号搜索和引用搜索，新增底部 **Layered Search** 面板，用 `Layered Text Search` 命令直接执行工作区文本搜索。

- 结果按 **工作区 → 目录树 → 文件 → 命中行** 展示，不再压平成 VS Code 原生搜索那种文件列表
- 使用本机 `rg`（ripgrep）执行固定字符串搜索，默认沿用 ripgrep 的忽略规则
- 点击命中行会在编辑器里预览并高亮对应位置
- 面板标题栏支持重新发起搜索和刷新当前查询
...
| `Ctrl+Shift+N` | Search Function / Method |
