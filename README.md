...
`Project Files` 会按工程文件树展示源码、测试和已忽略文件。当前目录链上如果每一级都只有**一个子目录且没有同级文件**，会自动压缩成一条紧凑路径，效果接近 IDEA 的 compact directories。
...
- 搜索弹窗右上角的原生按钮行直接提供常用选项：`include / exclude / Aa / ab / .* / 模糊 / 分组`；搜索结果视图标题栏保留 `搜索 / 刷新 / 顺序替换当前结果 / 撤销上次替换` 等动作入口
- 搜索结果视图右上角的 `include / exclude / Aa / ab / .* / 模糊` 会根据当前实际状态切换图标，便于直接确认哪些选项已生效
...
- 每一条替换都会写入 `IntelliJ-Style References` 输出通道，并带上唯一替换 ID，记录文件、行号、原命中、替换内容和失败原因，便于审计和回溯
